...
**Playback Modes** (`ShowConfig.playbackMode`):
The audio router supports two mutually exclusive modes. Ableton ignores arrangement view when session clips are fired, so these must not be mixed.
- **Session View** (`"session"`, default): Clips fired at slot 0 per row. The router tracks which clips have been fired.
- **Arrangement View** (`"arrangement"`): Global transport controls playback. No clips are fired. The router tracks whether transport has been started.
